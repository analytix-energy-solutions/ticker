...
class CategoryMixin:
    ...
    async def async_create_category(
        ...
        Args:
            ...
            bundle_on_release: When False, queued notifications released for this
                category are delivered individually (each held item keeps its own
                title/message/data) instead of collapsed into one summary. Sparse
                storage — persisted only when explicitly False; read-time default
                is True via category.get("bundle_on_release", True).
        """
